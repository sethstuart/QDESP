...
void processGamepad(ControllerPtr ctl) {
    ...
    // Handle misc buttons for menu
    if (std::find(pressedMiscButtons.begin(), pressedMiscButtons.end(), "center") != pressedMiscButtons.end()) {
        cycleEffects(); // Cycle to the next effect
    }

    // Run the current effect
    runCurrentEffect(brakeValue, throttleValue);
...
enum Effect {
    ...
    RAINBOW,
    POLICE_STROBE,
    DYNAMIC_BRAKE_THROTTLE,
    EFFECT_COUNT
};

Effect currentEffect = OFF;
int effectState = 0;
unsigned long lastUpdate = 0;
unsigned long effectInterval = 10; // Default interval

// Breathing white effect
void breathingWhite() {
    uint8_t brightness = (sin(millis() / 1000.0 * PI) * 127) + 128;
    pixel.setPixelColor(0, pixel.Color(brightness, brightness, brightness));
    pixel.show();
}

// Red and blue strobe effect
void policeStrobe() {
    if (effectState == 0) {
...
// Dynamic effect based on brake and throttle values
void dynamicBrakeThrottle(int brakeValue, int throttleValue) {
    int red = map(brakeValue, 0, 1023, 0, 255);
    int green = map(throttleValue, 0, 1023, 0, 255);
...
// Function to cycle through effects
void cycleEffects() {
    currentEffect = static_cast<Effect>((currentEffect + 1) % EFFECT_COUNT);
}

// Function to run the current effect
void runCurrentEffect(int brakeValue, int throttleValue) {
    unsigned long currentMillis = millis();
    if (currentMillis - lastUpdate >= effectInterval) {
        ...
        switch(currentEffect) {
            ...
            case POLICE_STROBE:
                ...
                policeStrobe();
                break;
            case DYNAMIC_BRAKE_THROTTLE:
                effectInterval = 10;
                dynamicBrakeThrottle(brakeValue, throttleValue);
                break;
            default:
                break;
        }
